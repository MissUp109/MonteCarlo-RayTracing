...
#include <iostream>
using namespace std;
Camera::Camera(int w, int h, Double3 pos, Double3 tar, double prj){
    width = w;
    height = h;
    position = pos;
    target = tar;
...
void Camera::setCamera(int w, int h, Double3 pos, Double3 tar, double prj){
    width = w;
    height = h;
    position = pos;
    target = tar;
...
Ray Camera::newRay(int x, int y){
    Double3 dir = normalize(direction + x_direction * (x - width / 2) * 15.0 / width + y_direction * (y - height / 2) * 15.0 / height);
    //cout << dir << endl;
